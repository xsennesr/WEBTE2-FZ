...
@section('content')
            ...
            <form action="{{ route('student.generate-task') }}" method="POST" class="mx-2">
                ...
                @foreach ($batches as $batch)
                    <div class="form-group mb-3">
                        ...
                        <input type="hidden" name="batch-id" id="batch-id" value="{{ $batch['name'] }}">
                    </div>
                @endforeach
    ...
    @else
        <div class="bg-light p-4 rounded">
            <label class="fs-4 text-decoration-underline mb-3">Nie su dostupné žiadne sady príkladov</label>
                    ...
                    <tbody>
                        @foreach ($tasks as $priklad)
                            <tr>
                                ...
                                <td>{{ $priklad->pivot->result ? 'Spravny' : ($priklad->result === false ? 'Nespravny' : 'Este ziadny') }}
                                </td>
